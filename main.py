...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/2012283888/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/2012283888')
   open(f"Users/2012283888/phone.csv","w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
APP_ID = 18020036
API_HASH = "c2d1495f5fad5e7562b25f5d6b7981cc"
BOT_TOKEN = "5186578540:AAHWzmvvY05hc6vbX6z5ZstvTcGMFgdLCxU"
UPDATES_CHANNEL = "BaghirovBlog"
OWNER= [2012283888]
PREMIUM=[2012283888]
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Siz Banlanmısınız. [Sahibim](https://t.me/TheBaghirov) ilə əlaqə saxlayın .", parse_mode="markdown", disable_web_page_preview=True)
            return 1
      except UserNotParticipant:
         await app.send_message(chat_id=message.chat.id, text="**Məni istifadə etmək üçün yeniliklər kanalına abunə olun!\n və /start üzərinə klikləyin**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤖 Join Updates Channel 🤖", url=f"https://t.me/{update_channel}")]]), parse_mode="markdown")
         return 1
      except Exception:
         await app.send_message(chat_id=message.chat.id, text="**Nə isə səhv getdi. Mənim [Sahibim](https://t.me/TheBaghirov) ilə əlaqə saxlayın.**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/2012283888/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/2012283888/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
